...
def get_weather_forecast(location: str, days: int = 3) -> Dict[str, Any]:
    ...
    This is a mock implementation designed for energy optimization agents.
    """

    if days < 1 or days > 7:
        raise ValueError("days must be between 1 and 7")

    def condition_for_hour(hour: int) -> str:
        if 6 <= hour <= 18:
    ...
    def solar_irradiance_for_hour(hour: int, condition: str) -> float:
        if hour < 6 or hour > 18:
            return 0.0

        # bell-shaped curve for solar production
        peak = 800  # W/m²
        irradiance = peak * max(
            0,
            1 - abs(hour - 12) / 6
    ...
    for hour in range(24):
        condition = condition_for_hour(hour)

        hourly_forecast.append(
            {
                "hour": hour,
                "temperature_c": round(
                    base_temp + random.uniform(-3, 3), 1
                ),
                "condition": condition,
                "solar_irradiance": solar_irradiance_for_hour(hour, condition),
                "humidity": random.randint(40, 80),
                "wind_speed": round(random.uniform(1, 8), 1),  # m/s
            }
        )
    ...
    forecast = {
        ...
        "forecast_days": days,
        "current": {
            "temperature_c": hourly_forecast[0]["temperature_c"],
            "condition": hourly_forecast[0]["condition"],
            "humidity": hourly_forecast[0]["humidity"],
            "wind_speed": hourly_forecast[0]["wind_speed"],
        },
...
def get_electricity_prices(date: str = None) -> Dict[str, Any]:
    ...
    This is a mock Time-of-Use (TOU) pricing model.
    """
    ...
    # Base prices (USD per kWh)
    off_peak_rate = 0.12
    shoulder_rate = 0.18
    peak_rate = 0.32

    # Demand charge applied only during peak hours
    peak_demand_charge = 0.08  # USD per kWh equivalent
